...
load('../data_expr1/datasets_expr1.mat')
...
% Clustering methods
% methodNames = {'k-DBA', 'k-Shape', 'k-GAK', 'k-SC', 'USSL', 'k-VARs (rnd)', 'k-VARs'};
methodNames = {'k-DBA', 'k-Shape', 'k-GAK', 'k-SC', 'k-VARs (rnd)', 'k-VARs (oracle)'};

%% Loading and formating results of clusterin precision
resExpr = cell(nPara, 0);

% load and compute results for kVARs
load('../result_expr1/kVARs_result.mat')
for iPara = 1:nPara
...
for iPara = 1:nPara
    ...
    % load raw results
    fname = [respath, 'kMeansDTW_m' num2str(m) '_result.txt'];
...
for iPara = 1:nPara
    ...
    % load raw results
    fname = [respath, 'kShape_m' num2str(m) '_result.txt'];
    groupExpr = readmatrix(fname, 'FileType', 'text', 'Delimiter', ',');
    groupExpr(~groupExpr) = 2;
...
for iPara = 1:nPara
    ...
    % load raw results
    fname = [respath, 'kMeansGAK_m' num2str(m) '_result.txt'];
    groupExpr = readmatrix(fname, 'FileType', 'text', 'Delimiter', ',');
    groupExpr(~groupExpr) = 2;
...
load('../result_expr1/kSC_result.mat')
for iPara = 1:nPara
...
xAxisNames = strread(num2str(mVec), '%s')';
xLabelName = 'time series dimensions (m)';
switch perfIndex
  case 'RI'
    yLabelName = 'RI';
  case 'NMI'
...
hold on
xl = xlim;
plot([1 1]*(xl(2)/3 + innerSpace), ylim, 'k:')
plot([1 1]*(xl(2)/3*2 + innerSpace), ylim, 'k:')
hold off
...
figname = ['boxplot_' perfIndex '.pdf'];
